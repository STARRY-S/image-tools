...
func (v *Validator) Validate(ctx context.Context, images ...string) error {
	var err error
	return v.cmd.Exec(ctx, images)

	var identities []cosign.Identity
	if v.cmd.KeyRef == "" {
		identities, err = v.cmd.Identities()
		if err != nil {
			return err
		}
	}
	...
	ref, err := name.ParseReference(images[0], v.cmd.NameOptions...)
	if err != nil {
		return fmt.Errorf("parsing reference: %w", err)
	}
	ref, err = sign.GetAttachedImageRef(ref, v.cmd.Attachment, ociremoteOpts...)
	if err != nil {
		return fmt.Errorf("resolving attachment type %s for image %s: %w", v.cmd.Attachment, images[0], err)
	}
